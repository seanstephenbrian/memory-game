...
function App() {
    ...
    useEffect(() => {
        ...
        if (localStorage.getItem('highScore') && parseInt(localStorage.getItem('highScore')) >= totalClicks) {
            // set high score in state to the localStorage high score:
        ...
        // OR if there's no high score saved locally...
            ...
            // set high score in state to the current # of clicks:
    ...
    if (!gameLost && !gameWon) {
        ...
        mainContent = <OutcomeMessage outcome='loss' handlePlayAgainClick={restartGame} />
    } else if (gameWon) {
        mainContent = <OutcomeMessage outcome='win' handlePlayAgainClick={restartGame} />
    }
